...
class AdminUnidadController extends Controller
{
    ...
    {
        $unidadAdmin = Auth::user()->unidad_id;

        //----contadores generales para el dashboard, filtrados por la unidad del admin
        $queryUnidad = function ($q) use ($unidadAdmin) {
            $q->where('unidad_id', $unidadAdmin);
        };

        $noAsignados = Ticket::whereHas('user', $queryUnidad)
            ->whereNull('tecnico_id')
            ->where('estado_id', '!=', 3)
            ->count();

        $pendientes = Ticket::whereNotNull('tecnico_id')
            ->whereHas('user', $queryUnidad)
            ->where('estado_id', '!=', 3)
            ->count();

        $resueltos = Ticket::whereHas('user', $queryUnidad)
            ->where('estado_id', 3)
            ->count();
        ...
        //----ultimos tickets
        $todosLosTickets = Ticket::with(['user', 'categoria'])
            ->whereHas('user', $queryUnidad)
            ->latest()
            ->paginate(25);
