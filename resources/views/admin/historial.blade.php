...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="rounded-2xl bg-white p-5 flex items-center justify-between shadow-sm border-t-4 border-orange-500 hover:translate-y-[-6px] transition-all duration-300">
        <div>
    ...
    <div class="rounded-2xl bg-white p-5 flex items-center justify-between shadow-sm border-t-4 border-primary hover:translate-y-[-6px] transition-all duration-300">
        <div>
    ...
    <div class="rounded-2xl bg-white p-5 flex items-center justify-between shadow-sm border-t-4 border-secondary hover:translate-y-[-6px] transition-all duration-300">
        <div>
...
<div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
    {{-- BARRA DE FILTROS --}}
    <div class="p-5 border-b border-slate-200 bg-slate-50/50 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
        <div class="space-y-1.5">
            <label class="text-[10px] font-black uppercase text-slate-500 tracking-wider">Identificación</label>
            <div class="relative">
                <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-lg">search</span>
                <input type="text" id="filtroBuscar" class="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:border-primary transition-all font-semibold" placeholder="ID, Usuario o Técnico...">
            </div>
        </div>
        <div class="space-y-1.5">
            <label class="text-[10px] font-black uppercase text-slate-500 tracking-wider">Fecha Inicio</label>
            <input type="date" id="filtroFechaInicio" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:border-primary transition-all font-semibold text-slate-600">
        </div>
        <div class="space-y-1.5">
            <label class="text-[10px] font-black uppercase text-slate-500 tracking-wider">Fecha Fin</label>
            <input type="date" id="filtroFechaFin" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:border-primary transition-all font-semibold text-slate-600">
        </div>
        <div class="space-y-1.5">
            <label class="text-[10px] font-black uppercase text-slate-500 tracking-wider">Estado Operacional</label>
            <select id="filtroEstado" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:border-primary transition-all font-black uppercase tracking-wider text-slate-600 cursor-pointer">
        ...
        <div class="space-y-1.5">
            ...
            <select id="filtroCategoria" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none focus:border-primary transition-all font-black uppercase tracking-wider text-slate-600 cursor-pointer">
        ...
        {{-- 🔥 BOTONES DE ACCIÓN Y EXPORTACIÓN AJUSTADOS --}}
        <div class="flex flex-col gap-1.5 w-full">
            <div class="flex gap-1.5 w-full">
                <button type="button" onclick="aplicarFiltrosHistorial()" class="flex-1 h-[36px] bg-secondary text-white rounded-xl text-[11px] font-black uppercase tracking-widest shadow-md hover:bg-opacity-95 transition-all flex items-center justify-center gap-1">
                    <span class="material-symbols-outlined text-sm">filter_alt</span> Filtrar
                </button>
                <button type="button" onclick="limpiarFiltrosHistorial()" class="w-[36px] h-[36px] bg-slate-200 text-slate-600 rounded-xl hover:bg-slate-300 transition-all flex items-center justify-center" title="Reiniciar Filtros">
                    <span class="material-symbols-outlined text-base">restart_alt</span>
                </button>
            </div>
            <div class="flex gap-1.5 w-full">
                <button type="button" onclick="exportarHistorial('pdf')" class="flex-1 h-[32px] bg-red-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm hover:bg-red-700 transition-all flex items-center justify-center gap-1">
                    <span class="material-symbols-outlined text-xs">picture_as_pdf</span> PDF
                </button>
                <button type="button" onclick="exportarHistorial('excel')" class="flex-1 h-[32px] bg-green-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm hover:bg-green-800 transition-all flex items-center justify-center gap-1">
    ...
    <div class="overflow-x-auto px-6 pb-6">
        <table id="tablaHistorial" class="w-full text-left border-separate border-spacing-0">
            <thead>
                <tr class="bg-slate-50 text-[12px] uppercase text-[#008F7E] font-black tracking-widest border-b border-slate-200">
