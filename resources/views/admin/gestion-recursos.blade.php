...
<div class="p-8 bg-slate-50 min-h-screen">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        @foreach($manuales as $manual)
            <div class="manual-card bg-white rounded-xl shadow-md overflow-hidden"
                data-categoria="{{ $manual->categoria_id }}">

                <div
                    class="h-24 bg-blue-900 relative p-4 bg-[url('https://www.transparenttextures.com/patterns/rocky-wall.png')]">
                    <span
                        class="relative z-10 bg-lime-500/20 text-lime-400 text-[14px] font-bold px-2 py-1 rounded border border-lime-500/50 uppercase">
                ...
                <div class="p-5">
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex gap-3 text-blue-900/60">
                            ...
                            <button onclick="eliminarManual({{ $manual->id }})" class="hover:text-red-600">
...
<div id="modalCrear"
    ...
    <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden">
        <div class="bg-primary p-6 text-secondary flex justify-between items-center">
            ...
            <button onclick="cerrarModal('modalCrear')" class="material-symbols-outlined font-bold">close</button>
        </div>
        <form id="formAgregar" action="{{ route('admin.manuales.store') }}" method="POST" enctype="multipart/form-data"
            class="p-6 space-y-4">
            @csrf
            <div>
...
<div id="modalEditar"
    ...
    <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden">
        ...
        <form id="formEditar" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            ...
            <div>
                <label class="text-[12px] font-black uppercase text-secondary">Titulo del Recurso</label>
                <input type="text" name="titulo" id="edit_titulo" class="w-full mt-1 p-3 bg-slate-50 border rounded-xl"
                    required>
            </div>
            <div>
                <label class="text-[12px] font-black uppercase text-secondary">Categoría</label>
                <select name="categoria_id" id="edit_categoria_id" class="w-full mt-1 p-3 bg-slate-50 border rounded-md"
                    required>
                    @foreach($categorias as $categoria)
                        <option value="{{$categoria->id}}">{{$categoria->nombre_categoria_manual}}</option>
                    @endforeach
            ...
            <div>
                <label class="text-[12px] font-black uppercase text-secondary">Reemplazar Archivo (PDF o MP4)</label>
...
<div id="modalCategorias"
    ...
    <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden">
        <div class="bg-primary p-6 text-secondary flex justify-between items-center">
            ...
            <button onclick="cerrarModal('modalCategorias')" class="material-symbols-outlined font-bold">close</button>
        </div>
        <form id="formAgregar" action="{{ route('admin.categorias.store') }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="nombre_categoria_manual" class="w-full mt-1 p-3 bg-slate-50 border rounded-xl"
                    required>
            </div>
            <button type="submit"
                class="w-full bg-secondary text-primary font-black py-3 rounded-xl shadow-lg mt-4 uppercase">Guardar
